...
if trained_model_exists:
    if completely_restart_train:
        ...
        if not (type(torch.load(save_model_path)) is collections.OrderedDict):
            ...
            elif try_resume_train and startEpoch >= num_epochs:
            ...
            elif not try_resume_train and startEpoch >= num_epochs:
...
if not trained_model_exists or try_resume_train or startEpoch < (num_epochs - 1):

    for t in range(startEpoch, num_epochs):
        print(f"Epoch {t+1}\n-------------------------------")
