...
def test(model_0, model_1, device, test_loader, epsilon, someSeed, detransform_func = lambda x: x):
    ...
    for data, target in test_loader:
        ...
        perturbed_data = [
            ...
            fgsm_attack(joint_data, epsilon, data_grad_0),
            fgsm_attack(joint_data, epsilon, data_grad_1),
            ]
    
        # Re-classify the perturbed image
        post_atk_output = [
            model_0(perturbed_data[0]),
            model_1(perturbed_data[1]),
            model_0(perturbed_data[2]),
            model_1(perturbed_data[3])
            ]
        
        # Check for success - # get the index of the max log-probability
        final_pred = [
            post_atk_output[0].max(1, keepdim=True),
            post_atk_output[1].max(1, keepdim=True),
            post_atk_output[2].max(1, keepdim=True),
            post_atk_output[3].max(1, keepdim=True),
        ]
        ...
        adv_examps = [
            perturbed_data[0].detach(),
            perturbed_data[1].detach(),
            perturbed_data[2].detach(),
            perturbed_data[3].detach()
            ]
        adv_examps_denormalized = [
            detransform_func(adv_examps[0]),
            detransform_func(adv_examps[1])] 
