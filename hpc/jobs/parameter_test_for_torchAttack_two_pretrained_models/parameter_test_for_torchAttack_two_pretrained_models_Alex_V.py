...
def test(model_0, model_1, device, test_loader, epsilon, someSeed, detransform_func = lambda x: x):
    ...
    adv_imgs_0 = [] ;  adv_imgs_1 = []
    adv_pred_labels_0 = [] ;  adv_pred_labels_1 = []
    adv_attack_labels_0 = [] ; adv_attack_labels_1  = []
    final_predictions_0 = [] ; final_predictions_1 = []
    ...
    for data, target in test_loader:
        ...
        # Call FGSM Attack
        perturbed_data = {
            "model_0_init_pred_indx" : fgsm_attack(data_0, epsilon, data_grad_0),
            "model_1_init_pred_indx" : fgsm_attack(data_1, epsilon, data_grad_1),
            "model_0_joint_pred_indx" : fgsm_attack(joint_data, epsilon, data_grad_0),
            "model_1_joint_pred_indx" : fgsm_attack(joint_data, epsilon, data_grad_1),
            }
    
        # Re-classify the perturbed image
        post_atk_output = {
            "model_0_init_pred_indx" : model_0(perturbed_data["model_0_init_pred_indx"]),
            "model_1_init_pred_indx" : model_0(perturbed_data["model_1_init_pred_indx"]),
            "model_0_joint_pred_indx" : model_0(perturbed_data["model_0_joint_pred_indx"]),
            "model_1_joint_pred_indx" : model_0(perturbed_data["model_1_joint_pred_indx"])
            }

        # Check for success - # get the index of the max log-probability
        final_pred = {
            "model_0_init_pred_indx" : post_atk_output["model_0_init_pred_indx"].max(1, keepdim=True),
            "model_1_init_pred_indx" : post_atk_output["model_1_init_pred_indx"].max(1, keepdim=True),
            "model_0_joint_pred_indx" : post_atk_output["model_0_joint_pred_indx"].max(1, keepdim=True),
            "model_1_joint_pred_indx" : post_atk_output["model_1_joint_pred_indx"].max(1, keepdim=True),
        }
        
        final_pred_index = {
            "model_0_init_pred_indx" : final_pred["model_0_init_pred_indx"][1],
            "model_1_init_pred_indx" : final_pred["model_1_init_pred_indx"][1],
            "model_0_joint_pred_indx" : final_pred["model_0_joint_pred_indx"][1],
            "model_1_joint_pred_indx" : final_pred["model_0_joint_pred_indx"][1],
            }
        
        adv_examps = {
            "model_0_init_pred_indx" : perturbed_data["model_0_init_pred_indx"].detach(),
            "model_1_init_pred_indx" : perturbed_data["model_1_init_pred_indx"].detach(),
            "model_0_joint_pred_indx" : perturbed_data["model_0_joint_pred_indx"].detach(),
            "model_1_joint_pred_indx" : perturbed_data["model_1_joint_pred_indx"].detach()
            }
        adv_examps_denormalized = [detransform_func(adv_examps[0]), detransform_func(adv_examps[1])] 
        
        adv_imgs_0.append(adv_examps_denormalized[0])
        adv_imgs_1.append(adv_examps_denormalized[1])
        
        adv_pred_labels_0.append(init_pred_index_0.detach())
        adv_pred_labels_1.append(init_pred_index_1.detach())
        
        adv_attack_labels_0.append(final_pred_index[0].detach())
        adv_attack_labels_1.append(final_pred_index[1].detach())

        final_predictions_0.append(final_pred_index[0].flatten().detach().cpu().numpy())
        final_predictions_1.append(final_pred_index[1].flatten().detach().cpu().numpy())
    ...
    final_predictions_0 = np.concatenate(final_predictions_0, axis=0)
    ...
    correct_0 = np.sum(final_predictions_0 == initial_predictions_0)
    correct_1 = np.sum(final_predictions_1 == initial_predictions_1)
    final_acc_0 = correct_0 / len(initial_predictions_1)
    ...
    adv_imgs_0 = torch.cat(adv_imgs_0, dim=0).cpu().numpy()
    adv_imgs_1 = torch.cat(adv_imgs_1, dim=0).cpu().numpy
